(async () => {
  try {
    ...
    const [baseResponse, patchResponse, midpointResponse, glideResponse, sampledResponse, touchResponse] = await Promise.all([
      ...
      fetch("app-v8-touch-wall.js", { cache: "no-store" })
    ]);
    if (!baseResponse.ok || !patchResponse.ok || !midpointResponse.ok || !glideResponse.ok || !sampledResponse.ok || !touchResponse.ok) {
    ...
    const touch = await touchResponse.text();
    await new Function(
      "HandLandmarker",
      "FilesetResolver",
      `return (async()=>{\n${base}\n${patch}\n${midpoint}\n${glide}\n${sampled}\n${touch}\n})()`
